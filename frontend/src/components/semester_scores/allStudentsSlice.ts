...
export const studentsSlice = createSlice({
  ...
  reducers: {
    ...
    updateStudent: (state, action: PayloadAction<StudentType>) => {
      const index = state.findIndex(student => student.google_id === action.payload.google_id);
      if (index !== -1) {
...
export const { addAllStudents, removeStudent, updateStudent } = studentsSlice.actions;
