    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            if section:
                ...
                courses = [
                    course
                    for course in classroom_analyzer.get_courses()
                    if course["section"] == section + "°"
                ]
                if not courses:
                    self.stdout.write(
                        self.style.WARNING(
                            f"No courses found with section number: {section}"
                        )
                    )
                    return
            else:
            ...
            if not courses:
                self.stdout.write(
                    self.style.WARNING("No courses found in Google Classroom.")
                )
            else:
            ...
            for course_obj in courses:
                ...
                course_record, created = Course.objects.get_or_create(
                    section=course_obj["section"], google_id=course_id
                )

                action = "Creating" if created else "Updating"
                self.stdout.write(
                    f"{action} course in database: {course_obj['section']} (ID: {course_id})"
                )

                classroom_analyzer.sync_student_data(course_id, course_record)
                self.stdout.write(
                    f"Synchronized data for course: {course_obj['section']} (ID: {course_id})"
                )

            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully synchronized data from Google Classroom {current_time}"
                )
            )
